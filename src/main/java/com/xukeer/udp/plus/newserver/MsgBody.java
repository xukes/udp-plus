...
public class MsgBody {
    ...
    public static int CROWD_LENGTH = 100; // 每个簇的消息数量
    ...
    public MsgBody(byte[] bytes) {
        ...
        this.crowdLength = (int) Utils.roundUpperNumbers(msgLength, CROWD_LENGTH); // 消息簇的格式
        msgCrowds = new MsgCrowd[crowdLength];
        for (int i = 0; i < crowdLength; i++) {
            int startIndex = i * MSG_LENGTH * CROWD_LENGTH;
            int endIndex = (i + 1) * MSG_LENGTH * CROWD_LENGTH;
            int crowdBytesLength;
            if (endIndex > length) {
                crowdBytesLength = (int) (length - startIndex);
            } else {
                crowdBytesLength = MSG_LENGTH * CROWD_LENGTH;
            }
    ...
    public Iterator<MsgCrowd> getIterator() {
        Iterator<MsgCrowd> iterator = new Iterator<MsgCrowd>() {
            ...
            public MsgCrowd next() {
                ...
                byte[] arr = new byte[CROWD_LENGTH * MSG_LENGTH];
                try {
