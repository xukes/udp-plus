...
import com.xukeer.udp.utils.Utils;

import static com.xukeer.udp.newserver.MsgBody.MSG_LENGTH;
...
public class MsgCrowd {
    ...
    public MsgCrowd(int sequence, byte msgType, int totalCrowd, int index, byte[] bytes) {
        ...
        this.length = (int) Utils.roundUpperNumbers(byteLength, MSG_LENGTH);
        msgBodies = new SimpleMsgBody[length];
        for (int i = 0; i < length; i++) {
            int startIndex = i * MSG_LENGTH;
            int endIndex = (i + 1) * MSG_LENGTH;
            int crowdBytesLength;
            if (endIndex > byteLength) {
                crowdBytesLength = byteLength - startIndex;
            } else {
                crowdBytesLength = MSG_LENGTH;
            }
